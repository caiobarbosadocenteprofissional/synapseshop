...
[x] A modelagem relacional de pelo menos uma entidade (ex: User, Token ou Pedido) foi concluída e validada.

[x] Os índices essenciais foram definidos na modelagem do banco de dados.

[x] As regras de integridade relacional foram estabelecidas e aplicadas.

[x] As migrações do banco de dados foram criadas e executadas com sucesso utilizando SQLAlchemy e Alembic no PostgreSQL.

[x] O versionamento do schema do banco de dados foi implementado.

[x] Um rollback seguro de migração foi demonstrado e validado pela equipe.

[x] Os repositórios transacionais foram implementados de acordo com a arquitetura do projeto.

[x] Um serviço que consome os repositórios transacionais foi construído.

[x] Testes transacionais iniciais foram realizados para garantir o funcionamento da persistência.

[x] Os tempos de execução das consultas e transações foram coletados.

[x] O repositório foi atualizado com o registro de todas as decisões técnicas tomadas durante a modelagem e testes.

[x] A regra de não-antecipação foi respeitada, garantindo que recursos de aulas futuras não foram codificados nesta etapa.