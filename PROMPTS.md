...
### 2026-09-23 — Equipe SynapseShop — opencode
- **Contexto:** Aula 7 — planejamento da camada de autenticação JWT com papéis e throttling.
- **Prompt:** "Seguindo as skills e a diretriz incremental, execute a specs_da_aula_7 (JWT com papéis admin/user, proteção das rotas administrativas, throttling, paginação/filtros, coleção Postman) sem antecipar aulas futuras (Redis, mensageria, pytest, IA)."
- **Resultado / Ajustes:** Plano incremental definido com decisões esclarecidas com a squad: (1) item 1 da seção 3 ("índices compostos/rollback/repositório transacional") é artefato da Aula 6 — não executado; (2) JWT aplicado somente na API Django/DRF; (3) `User` customizado (`AbstractUser`) com campo `role`. Dependência pinada `djangorestframework-simplejwt==5.5.*` (v5.5.1). Intervenção manual: definidos escopos de throttling `anon`/`user`/`login`, tempo de vida do JWT via env e usuários demo (seed) para reproduzir os cenários do Postman.

### 2026-09-23 — Equipe SynapseShop — opencode
- **Contexto:** Aula 7 — implementação da camada de acesso: modelo de usuário com papéis, fluxos JWT, throttling, permissões, paginação e filtros.
- **Prompt:** "Crie o model User customizado com role (admin/user), serializers/views JWT com claim role, permission IsAdminRole (leitura autenticada, escrita admin-only), throttling escopado em login e paginação/filtros nos ViewSets — sem antecipar Redis (Aula 8), mensageria (Aulas 9-11) ou pytest (Aula 12)."
- **Resultado / Ajustes:** `repositories/models.py` ganhou `User(AbstractUser)` com `role`; migração `0002_user` gerada com `makemigrations` (auth nativo do Django habilitado; `AUTH_USER_MODEL` definido antes da 1ª execução). Criados `api/auth.py` (serializers com claim `role`, views com `ScopedRateThrottle`/escopo `login`) e rotas `/api/v1/auth/token/` e `/refresh/`. `api/views.py` com `IsAdminRole`, `SearchFilter`/`OrderingFilter` e filtros `is_active`/`category`. `settings.py` com `SIMPLE_JWT`, paginação e taxas via env. Comando `seed_demo_users` idempotente rodado no start do container. Intervenção manual: `is_staff`/`is_superuser` sincronizados ao papel no seed.

### 2026-09-23 — Equipe SynapseShop — opencode
- **Contexto:** Aula 7 — validação funcional, coleção Postman e documentação (DoD).
- **Prompt:** "Valide os fluxos JWT (200/401/403/429), paginação e filtros; crie a coleção Postman da Aula 7 e registre decisões, métricas, README e PROMPTS — mantendo o escopo SpecDD."
- **Resultado / Ajustes:** Smoke test reproduzível `scripts/smoke_test_auth.py` validou **13/13 PASS** contra o container Docker: login admin/user, erro 401, refresh com `role`, GET sem token 401, POST user 403, CRUD admin 201/204, filtros/paginação e 429 no 6º login. Coleção `docs/postman/SynapseShop_Aula7.postman_collection.json` (6 pastas com asserts). Criados `docs/DECISOES_TECNICAS_AULA7.md` e `docs/METRICAS_AULA7.md`; README com a seção 9 (autenticação/throttling/paginação), tabelas de envs e de docs; DoD da spec marcado como cumprido. Intervenção manual: rebuild da imagem para incluir a migração gerada; validação também em `runserver` local.

### 2026-09-25 — Equipe SynapseShop — opencode
- **Contexto:** Aula 7 — correção da camada de dados: a autenticação rodava sobre SQLite em vez do PostgreSQL já provisionado pelo Compose.
- **Prompt:** "Estamos fazendo autenticação da forma incorreta de acordo com a arquitetura definida. Troque o sqlite pelo postgresql do container docker que já está definido pelo projeto."
- **Resultado / Ajustes:** Diagnóstico: `config/settings.py` mantinha `ENGINE: sqlite3` hardcoded e ignorava as variáveis `POSTGRES_*` que o Compose já injetava no serviço `api` — o serviço `postgres` (com healthcheck e volume `pgdata`) ficava ocioso, e `User`, `role`, seed e a resolução do token liam um `db.sqlite3` descartável sem volume, contradizendo a Camada 5 da arquitetura-alvo. `DATABASES` trocado para `django.db.backends.postgresql` lendo as cinco `POSTGRES_*` (default de código `localhost`, sobrescrito para `postgres` pelo Compose, espelhando `inventory/app/db.py`); `psycopg[binary]==3.3.*` pinado no `requirements.txt` (mesmo pin do `inventory`); `docker-compose.yml` **sem alteração** — as envs e o `depends_on: service_healthy` já estavam prontos. Nenhuma migração nova: `0001_initial` e `0002_user` aplicaram sem edição. README corrigido (§7, §8, §9, tabela de envs), decisão registrada em `DECISOES_TECNICAS_AULA7.md` §12, nota de revisão em `DECISOES_TECNICAS_AULA6.md` §1 e evidências em `METRICAS_AULA7.md` §6. Escopo mantido: nenhuma correção adjacente de auth. Validação: build com `psycopg_binary-3.3.6` manylinux instalado offline; 3 containers saudáveis; 12 tabelas em `psql` (Django + `inventory_items` no mesmo banco); `repositories_user` com `admin`/`user`; nenhum `.sqlite` no contêiner; **smoke test 13/13 PASS**; 401 sem token, 200 com `Bearer`. Intervenção manual: `db.sqlite3` ficou travado no Windows (DBeaver aberto em `repositories_user`, PID 7320) e não pôde ser removido nesta sessão.
